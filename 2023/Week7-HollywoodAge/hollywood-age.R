...
age_gaps <- age_gaps %>%
  clean_names(case = "lower_camel", abbreviations = c("ID", "DOB")) # abbreviations argument does tell {clean_names} to leave certain abbreviations untouched:
...
ggplot(age_gaps, aes(x = age_man, y = age_woman, color = period, alpha = 0.5)) +
  ...
  geom_smooth(linetype = "dashed", color = "lightblue", alpha = 0.7) +
